...
##               [xlo, xhi, ylo, yhi, zlo, zhi, n_links, n_chains, links, chains, atom_types, bond_types, mass, loop_atoms, G]=ioLAMMPS.readLAMMPS_restart("./"+str(round(frac_weak*100))+"/lambda_"+str(int(lam_max))+"/restart_network_0.txt", vflag,frac_weak,lam_max)
               G=nx.Graph()
               [xlo, xhi, ylo, yhi, zlo, zhi, n_links, n_chains, links, chains, atom_types, bond_types, mass, loop_atoms]=ioLAMMPS.readLAMMPS("./"+str(round(frac_weak*100))+"/network.txt", vflag,frac_weak)##,G)
...
##               ite_arr=np.arange(10,n_chains*0.8,50,dtype='int')
               ...
               ite_arr=np.arange(300,700,max(p.wrt_step,5),dtype='int')##p.wrt_step
...
########                                   tmp=tmp+1
########                                   if(count-cnt_orig!=1):
########                                           a_break_1=cnt_orig # first point where to break the data
########                                           break
########                                   else:
########                                           cnt_orig=count
########                           if(tmp==len(a[0])-1): # -1 because we are considering the count from a[0][1:]
########                                   a_break_1=a[0][0]
########                                   a_break_2=a[0][-1]
########                           else:
########                                   cnt_orig=a[0][tmp]
########        ##                           for count in a[0][-2::-1]:
########                                   for count in a[0][tmp:]:
########                                           if(count-cnt_orig!=-1):
########                                                   a_break_2=cnt_orig # second point where to break the data
########                                                   break
########                                           else:
########                                                   cnt_orig=count
########                           x_1=xrange[0:-1][a_break_1]
########                           x_2=xrange[0:-1][a_break_2]
########                           width[cnt]=x_2-x_1
########                           print('width[cnt]',width[cnt])
##########                           plt.show()
########                   else:
########                           width[cnt]=0
